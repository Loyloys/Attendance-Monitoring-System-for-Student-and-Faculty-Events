...
export default function QRCodeGenerator({ lecturerId = 'LEC001', courseId, onQRGenerated }: QRCodeGeneratorProps) {
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      const url = await QRCode.toDataURL(session.qrData, {
        width: 400,
        margin: 2,
        color: { dark: '#F97316', light: '#FFFFFF' } // COT Orange QR
      });
      setQrCodeUrl(url);
  ...
  const selectBaseStyle = "w-full pl-11 pr-10 py-3.5 bg-white/50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-[#F97316]/5 focus:border-[#F97316] transition-all appearance-none font-medium text-slate-700";
        ...
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2">
          <div className="relative space-y-2 group">
            ...
            <div className="relative">
              <BookOpen className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#F97316]" size={18} />
              <select
          ...
          {selectedCourse && (
            <div className="relative space-y-2 group animate-in zoom-in-95">
              ...
              <div className="relative">
                <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#F97316]" size={18} />
                <select
        ...
        <div className="bg-white rounded-[40px] border border-slate-100 shadow-2xl p-8 lg:p-12 text-center relative overflow-hidden animate-in slide-in-from-top-8">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <span className="px-4 py-1.5 bg-[#EA580C]/10 rounded-full text-xs font-bold text-[#EA580C] uppercase tracking-widest">{currentSession.location}</span>
            </div>
          </div>

          {/* High Contrast QR Area */}
          <div className="relative inline-block p-6 bg-white rounded-[32px] border-4 border-[#F97316]/10 shadow-inner group transition-transform hover:scale-[1.02]">
            ...
            <div className="absolute flex items-center gap-2 px-6 py-2 -translate-x-1/2 bg-white border rounded-full shadow-lg -bottom-4 left-1/2 border-slate-100">
               <div className={`w-2 h-2 rounded-full animate-ping ${timeRemaining < 300 ? 'bg-red-500' : 'bg-orange-500'}`} />
          ...
          <div className="grid max-w-2xl grid-cols-1 gap-4 mx-auto my-10 sm:grid-cols-3">
            ...
            <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl">
              <Timer className="text-[#F97316]" size={20} />
          ...
          <div className="flex flex-col max-w-md gap-4 mx-auto sm:flex-row">
            <Button 
              ...
              onClick={() => {
                ...
                link.download = `COT-QR-${currentSession.id}.png`;
                link.href = qrCodeUrl;
                link.click();
              }}
            >
          ...
          <div className="mt-8 p-4 bg-[#F97316]/5 rounded-2xl flex items-start gap-3 text-left border border-[#F97316]/10 max-w-xl mx-auto">
            <Info className="text-[#F97316] shrink-0 mt-0.5" size={18} />
            <p className="text-xs font-medium leading-relaxed text-slate-600">
              Display this QR on the projector. Students must use the <strong>COT Attendance Web</strong> app to scan. Attendance records are synced in real-time with the Faculty Admin portal.
            </p>
          </div>
        </div>
      )}
